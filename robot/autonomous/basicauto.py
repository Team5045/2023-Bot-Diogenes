...
class autoCharge(AutonomousStateMachine):
    ...
    boom_arm: Boom
    solenoid_toggle: Grabber
    
    @timed_state(duration = 3, next_state = "ExtendArm",  first = True)
    def RaiseArm(self):
    ...
    def retract(self):
        self.boom_arm.set_rotator(0.15)
        self.boom_arm.set_extender(0.25)
        self.sd.putValue("Mode: ", "Retracting extender and rotation")

    @timed_state(duration = 1, next_state = "Done")
    def Moveback(self):
        self.boom_arm.set_extender(0.0)
        self.boom_arm.set_rotator(0.0)
        self.drivetrain(-0.5, 0.0)
        self.sd.putValue("Mode: ", "going to Charge Station")

    @state
    def Done(self):
        self.drivetrain(0.0, 0.0)
